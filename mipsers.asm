...
copyByteInSuff:
	...
	BNEC R1, R2, copySuff ; check if index r1 contains '$'
...
suffixInnerCompareLoop:
	...
	BEQZC R28, suffixEndInnerCompareLoop
...
findSuffix2:
	...
	LB R7, SUFF(R5)
	BEQZC R7, suffixEndInnerCompareLoop ; if zero
	BEQC R7, R25, findNextNonEmpty
	BNEC R7, R30, findSuffix2
...
compareSuffix1and2:
	...
	BNEZC R9, suffix1LesserThan2 ; if curr index value suffix 1 less than suffix 2's
	BEQC R3, R7, suffix1And2NextIndex ; if values are equal

	BC suffix1GreaterThan2 ; if curr index value suffix 1 greater than suffix 2's

suffix1LesserThan2:
	DADDIU R5, R6, 0x0000 ; init search for next suffix, start with the most recent index
	BC suffixInnerCompareLoop
...
removeSuffix:
	...
	BEQZC R3, suffixOuterCompareLoop
	BEQC R3, R30, suffixOuterCompareLoop	
